...
const Dashboard = () => {
  ...
  const postAbsenteeList = (event) => {
    ...
    postAbsentee(idtoken, { meal: absentee }).then((data, err) => {
  ...
  const postStudentFeedback = (event) => {
    ...
    postFeedback(idtoken, { meal, rating, review }).then((data, err) => {
      if (err) console.log(err);
      else {
  ...
  return (
    <div>
      ...
      {isAuthenticated() ? <div>Authenticated user</div> : null}
    </div>
  );
};
